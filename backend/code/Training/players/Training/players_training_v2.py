...
DEFAULT_DATA_ROOT = "/userhome/cs/u3597462/Autoscout/Data/players/Training2"
DEFAULT_OUT_DIR = "/userhome/cs/u3597462/Autoscout/AutoScout_Server/runs"
DEFAULT_RUN_NAME = "players-det-v2"
DEFAULT_MODEL = "yolov8n.pt"  # YOLO detection (boxes)
...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Fine-tune YOLOv8 detection on Players dataset.")
	parser.add_argument("--data", type=str, default=DEFAULT_DATA_ROOT, help="Dataset root directory or YAML path.")
	parser.add_argument("--out", type=str, default=DEFAULT_OUT_DIR, help="Output project directory (runs root).")
	...
	parser.add_argument("--model", type=str, default=DEFAULT_MODEL, help="Base model weights, e.g. yolov8n.pt.")
	parser.add_argument("--weights", type=str, default="", help="Optional checkpoint (.pt) to start from instead of --model.")
	parser.add_argument("--resume", action="store_true", help="Resume from latest 'weights/last.pt' under --out if found.")
	parser.add_argument("--device", type=str, default="auto", help="Device: '0'.. for GPU, 'cpu', or 'auto'.")
	parser.add_argument("--seed", type=int, default=0, help="Random seed.")

	# Common augmentation knobs (tunable)
...
def main() -> None:
	...
	if args.resume:
		...
		if last_ckpt:
			...
			print("[warn] --resume set but no checkpoint found; starting new run.")
	elif args.weights:
